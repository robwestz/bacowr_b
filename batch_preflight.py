...
def _run_with_db(pipe, jobs, db) -> list:
    """Check preflight DB for cached data, fall back to live fetch for misses."""
    from models import Preflight, RiskLevel
    from pipeline import detect_language, assess_risk
    ...
    for job in jobs:
        cached = db.get_preflight(job.publisher_domain, job.target_url, job.anchor_text)
        if cached and not db.is_stale("publisher", job.publisher_domain) and not db.is_stale("target", job.target_url):
            cached.job = job
            all_preflights.append(cached)
            db_hits += 1
        else:
            live_jobs.append(job)

    if live_jobs:
        live_preflights = asyncio.run(pipe.run_batch_preflight(live_jobs))
        for pf in live_preflights:
            if pf.publisher:
                db.save_publisher(pf.publisher)
            if pf.target:
                db.save_target(pf.target)
            all_preflights.append(pf)

    all_preflights.sort(key=lambda p: p.job.job_number)
    print(f"DB hits: {db_hits}, live fetches: {len(live_jobs)}")
    return all_preflights
...
def main() -> None:
    ...
    parser.add_argument(
        "--use-db",
        action="store_true",
        help="Check preflight_db for cached publisher/target data before live fetch.",
    )
    parser.add_argument(
        "--db-dir",
        default="preflight_db",
        help="Path to preflight database directory (used with --use-db).",
    )
    ...
    if args.use_db:
        try:
            from preflight_db.db import PreflightDB, PreflightDBConfig
            db = PreflightDB(PreflightDBConfig(db_dir=args.db_dir))
            print(f"Using preflight DB at {args.db_dir}")
        except Exception as exc:
            print(f"WARNING: Could not load preflight DB ({exc}), falling back to live fetch")
            db = None

    if db:
        all_preflights = _run_with_db(pipe, jobs, db)
    else:
