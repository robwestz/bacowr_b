# BACOWR v6.2 — Flowmap (Technical Specification, upgraded 2026-02-19)
...
Core runtime files divide responsibility:
...
| **models.py** | Shared dataclasses (`JobSpec`, `Preflight`, `PublisherProfile`, `TargetFingerprint`, `SemanticBridge`) | No logic — pure data structures |
| **batch_preflight.py** | Batch Phase 1-4 export to JSON contracts (`job_NNN.json`, `batch_manifest.json`) | Web searching, article writing |
| **batch_blueprint.py** | Batch blueprint + prompt generation from enriched preflight JSON | SERP execution |
| **integration_test.py** | End-to-end structural gate (pipeline→engine→validator + deprecation checks) | Live web research |
...
**Execution profiles (both supported):**
- **Per-job profile:** Agent runs Phases 1-8 in sequence for each job.
- **Batch profile (recommended):** `batch_preflight.py` handles Phases 1-4 in bulk, agent executes SERP phase, then `batch_blueprint.py` generates all prompts in bulk.
...
# Or bulk:
# preflights = asyncio.run(pipe.run_batch_preflight(jobs))
```
...
**Failure mode:** Without `aiohttp`/`beautifulsoup4`, target may be thin and distance defaults to 0.5. This is expected — Phase 3 compensates.  
If embeddings cannot load (memory/proxy/network), pipeline now degrades gracefully to heuristic distance instead of crashing.
...
# Returns: List[str], max 2 queries
# Query pattern: "{trust_link_topic} rapport forskning"
...
**Action:** Run deterministic validator (`article_validator.py`) aligned with qa-template.md — 11 binary checks:
...
## 3.5 Batch Scripts (Implemented Upgrade Path)

### `batch_preflight.py` (Phases 1-4 in bulk)
...
- job identity (`job_number`, `publisher_domain`, `target_url`, `anchor_text`)
- serialized preflight (`publisher`, `target`, `bridge`, language/risk/warnings)
- `probes` (if metadata exists) and `status` (`ready_for_serp` or `needs_metadata`)

### `batch_blueprint.py` (Blueprints + prompts in bulk)

```bash
python batch_blueprint.py preflights --output-dir blueprints
```

Readiness contract per job:
- `preflight.target.title` non-empty
- `probes_completed >= 3` (from `probes`/`target_intent`)
...
### Operational sequence (batch mode)

1. Run `batch_preflight.py`
2. Patch missing metadata (`target.title`, `target.meta_description`) where needed
3. Execute SERP probes + trustlink discovery per job (agent-driven)
4. Persist enriched `target_intent` (`probes_completed >= 3`)
5. Run `batch_blueprint.py`
6. Write articles + run validator (11/11 gate)

---

## 3.6 Legacy Paths and Deprecations

- `PromptGenerator.generate()` is deprecated.
- `Pipeline.save_prompt()` is deprecated.
- Legacy duplicate `text-output/qa_check.py` is deprecated (use `article_validator.py`).

These paths are retained for compatibility but should not be used in new production flows.

---

## 3.7 Integration Gate
...
The gate verifies:
- Pipeline load/preflight path
- Probe plan generation (5 probes)
- Blueprint generation and constraint checks
- Trustlink validator edge behavior
- Deprecation warning behavior

Readiness shortcut:

```bash
python integration_test.py && python -c "import engine; import article_validator; print('OK')"
```
...
Modify **only**: `TargetIntentAnalyzer.build_trustlink_queries()` and `select_trustlinks()` in engine.py.
Do **not** touch: `SemanticBridge.trust_link_topics` generation in pipeline.py (that feeds the queries).
The contract: `build_trustlink_queries` returns `List[str]` (max 2), `select_trustlinks` returns `List[Dict]` sorted best-first.
...
### "I want to change batch contracts or manifests"
Modify **only**:
- `batch_preflight.py` (job JSON + `batch_manifest.json`)
- `batch_blueprint.py` (`batch_ready.json`, prompt/blueprint filenames)

Do **not** change ad-hoc JSON keys in one script only — keep both scripts and operator docs aligned.

### "I want to change integration pass criteria"
Modify **only**: `integration_test.py` and this `FLOWMAP.md` section.
Keep the gate deterministic (no live web dependency).

---

*BACOWR v6.2 — upgraded 2026-02-19*
*Specification, not narrative. Signatures and operational gates aligned with current repo scripts.*
