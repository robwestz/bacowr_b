...
class TargetIntentAnalyzer:
    ...
    def _synthesize(self, profile: TargetIntentProfile) -> TargetIntentProfile:
        ...
        # Entities to avoid (competitor-specific, not topic-relevant)
        profile.entities_to_avoid = []  # Populated by competitor filter
    ...
    def build_trustlink_queries(
        ...
        """Generate 2 search queries for finding trust link candidates via web_search.

        Args:
            preflight_bridge: SemanticBridge from pipeline preflight (has .trust_link_topics)
            plan: TargetIntentProfile from SERP research (has .head_entity)
            target_title: The target page's title as fallback
        """
        queries: List[str] = []
        if preflight_bridge and getattr(preflight_bridge, "trust_link_topics", None):
            for t in preflight_bridge.trust_link_topics[:2]:
                if t:
                    queries.append(f"{t} rapport forskning")
        if not queries and plan and plan.head_entity:
            queries.append(f"{plan.head_entity} rapport forskning")
        if not queries and target_title:
            queries.append(f"{target_title} rapport")
...
class TopicDiscoveryEngine:
    ...
    # Common bridge patterns by vertical pair (expanded from SemanticEngine)
    BRIDGE_PATTERNS: Dict[Tuple[str, str], List[Dict[str, str]]] = {
        ("sport", "casino"): [
...
class SectionPlanner:
    ...
    def plan(
        ...
        """Generate section plan with constraint-optimized allocation.

        Final allocation (Claude+Codex converged):
            HOOK=125, ESTABLISH=140, DEEPEN=135, ANCHOR=155, PIVOT=145, RESOLVE=125

        Design properties:
            sum = 825
            words_before_anchor = 125+140+135 = 400 (symmetric C2 margin: 150/150)
            min section = 125 → worst-case at -15% = 106 > 80 (C6)
            HOOK/RESOLVE = 125 → ±15% gives 106–144, within C7 band [100,160]
            Trust links in sections 2+3 (before anchor section 4) → C4/C5 deterministic
        """
        sections = []

        primary_bridge = next(
            (b for b in bridges if b.role == BridgeRole.PRIMARY),
            bridges[0] if bridges else None,
        )
        supporting_bridge = next(
            (b for b in bridges if b.role == BridgeRole.SUPPORTING),
            None,
        )

        # Section 1: Hook — 125 words
        sections.append(SectionPlan(
            ...
            target_words=125,
            entities_to_cover=topic.bridges_available[0].extracted_facts[:2] if topic.bridges_available and topic.bridges_available[0].extracted_facts else []
        ))

        # Section 2: Establish — 140 words [trust link 1]
        sections.append(SectionPlan(
            order=2,
        ...
        # Section 3: Deepen — 135 words [trust link 2]
        sections.append(SectionPlan(
            order=3,
            role=SectionRole.DEEPEN,
            working_title="[Agent bestämmer – ny insikt som bygger på sektion 2]",
            purpose="Fördjupar med specifik data, exempel eller analys. Stärker tesen genom nyansering eller komplikation. Andra trustlänken placeras här som stöd.",
        ...
        # Section 4: Anchor — 155 words [anchor link]
        sections.append(SectionPlan(
            order=4,
        ...
        # Section 5: Pivot — 145 words (no links)
        sections.append(SectionPlan(
            order=5,
...
class RedThreadValidator:
    ...
    def validate(
        ...
        # Check 2: Swappability (simplified – checks role ordering)
        swappable = []
        for i in range(len(sections) - 1):
            s1, s2 = sections[i], sections[i + 1]
            # If two consecutive sections have same role, they might be swappable
...
class ConstraintEnforcer:
    ...
    def check_blueprint(
        ...
        # H1: Target word count in valid range (both bounds)
        total_target = sum(s.target_words for s in blueprint.sections)
        results.append(ConstraintResult(
            name="target_word_count",
            constraint_type=ConstraintType.HARD,
            passed=750 <= total_target <= 900,
            value=total_target,
            expected="750-900",
            message=f"Planned total: {total_target} words (target: 750-900)"
        ))

        # H1b: Every section meets minimum floor (C6: >=80 words)
        section_floor_ok = all(s.target_words >= 80 for s in blueprint.sections)
        under_floor = [f"s{s.order}={s.target_words}" for s in blueprint.sections if s.target_words < 80]
        results.append(ConstraintResult(
            name="section_floor",
            constraint_type=ConstraintType.HARD,
            passed=section_floor_ok,
            value=str(min(s.target_words for s in blueprint.sections)) if blueprint.sections else "N/A",
            expected=">=80 per section",
            message="All sections >=80 words" if section_floor_ok else f"VIOLATION: sections under floor: {under_floor}"
        ))

        # H1c: HOOK and RESOLVE within 100-160 band (C7)
        hook_resolve = [s for s in blueprint.sections if s.role in (SectionRole.HOOK, SectionRole.RESOLVE)]
        hook_resolve_ok = all(100 <= s.target_words <= 160 for s in hook_resolve)
        out_of_band = [f"{s.role.value}={s.target_words}" for s in hook_resolve if not 100 <= s.target_words <= 160]
        results.append(ConstraintResult(
            name="hook_resolve_band",
            constraint_type=ConstraintType.HARD,
            passed=hook_resolve_ok,
            value=", ".join(f"{s.role.value}={s.target_words}" for s in hook_resolve),
            expected="100-160",
            message="HOOK/RESOLVE within band" if hook_resolve_ok else f"VIOLATION: out of 100-160 band: {out_of_band}"
        ))
        ...
        # H5b: ALL trust links BEFORE anchor section (C4)
        trust_after_anchor = [
            s for s in blueprint.sections
            if s.contains_bridge and anchor_section and s.order >= anchor_section.order
        ]
        all_trust_before = len(trust_after_anchor) == 0
        results.append(ConstraintResult(
            name="trust_links_before_anchor",
            constraint_type=ConstraintType.HARD,
            passed=all_trust_before,
            value=f"{len(trust_after_anchor)} trust section(s) at/after anchor",
            expected="0",
            message="All trust links before anchor" if all_trust_before else f"VIOLATION: {len(trust_after_anchor)} trust link section(s) at or after anchor section"
        ))
...
class AgentPromptRenderer:
    ...
    def render(blueprint: ArticleBlueprint) -> str:
        ...
        # Extract target domain for trust link constraint display
        from urllib.parse import urlparse
        try:
            target_domain = urlparse(blueprint.target_url).netloc.replace("www.", "").lower()
        except Exception:
            target_domain = blueprint.target_url
...
"""
        ...
        if blueprint.target and blueprint.target.intent_profile:
            ...
            if tl_queries:
                ...
                trustlink_section += f"""
INSTRUKTION: Välj 1-2 resultat som:
...
### Ordantal
Sikta mot 825 ord. Håll dig inom 790–860 ord under utkastet och finjustera till 750–900 före leverans.

Riktmärke per stycke (kumulativt): stycke 1 slutar runt ord 125. Stycke 2 runt ord 265. Stycke 3 runt ord 400 — här bör ankarlänken sitta. Stycke 4 runt ord 555. Stycke 5 runt ord 700. Stycke 6 avrundas vid 810–840 ord.

Hela artikeln: 52–58 meningar (svenska meningar ≈ 15 ord i snitt). Stycken 100–200 ord var, fullt utvecklade tankar. Inga korta ensamma meningar som egna stycken.
...
Placera i stycke 3 eller 4 (runt ord 380–430). VARIERA placeringen per artikel.
...
1-2 verifierade källhänvisningar, alla FÖRE ankarlänken i artikelflödet.
ALDRIG till konkurrenter, affiliatesajter, {target_domain} eller {blueprint.publisher_domain}.
...
### Förbjudna fraser
{forbidden_str}
Referera aldrig till "denna artikel" eller "i denna text".

### CHECKLISTA — Ha detta i åtanke under hela skrivandet

ANKARLÄNK: Exakt en ankarlänk i hela texten, kopierad exakt: [{blueprint.anchor_text}]({blueprint.target_url}). Placera den i stycke 3 eller 4 (runt ord 380–430). Ankarlänken får ALDRIG upprepas.

TRUSTLÄNKAR: 1-2 trustlänkar, alla FÖRE ankarlänken. Aldrig till målsidans domän eller publisherns domän. Varje trustlänk ska vara en djuplänk.

FORMAT: EN rubrik (titel), sedan enbart prosa. Inga H2, H3 eller underrubriker. Inga punktlistor, inga numrerade listor. Minst 4 stycken, vardera 100–200 ord.

ENTITETER: Väv in minst 5 SERP-bekräftade entiteter naturligt i texten.

SPRÅK: Skriv hela artikeln på {blueprint.language}. Blanda inte in engelska fraser (utom facktermer).

FÖRBJUDNA MÖNSTER: Använd ALDRIG: "i en värld där", "det är viktigt att notera", "sammanfattningsvis", "i slutändan", "i dagens digitala värld", "har du någonsin undrat", "i takt med att", "det kan konstateras att".
