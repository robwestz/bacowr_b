...
| `batch_preflight.py` | Batch export av preflights + probe-plan till JSON |
| `batch_blueprint.py` | Batch generation av blueprints + skrivprompter |
| `integration_test.py` | Integrationsgate för pipeline→engine→validator |
...
Kör sedan integrationsgate innan större batchkörning:
...
**Zero-context?** Läs **ZERO_CONTEXT_LOAD.md** först — den ger exakt läsordning.
...
### 2c (rekommenderad drift): Script-baserad batch preflight

För operativ körning rekommenderas scriptet nedan eftersom det skriver stabila JSON-kontrakt direkt:
...
Jobb med `status=needs_metadata` patchas först (`target.title`, `target.meta_description`) innan SERP-steget.
...
Skriv artikeln till disk med SYSTEM.md-regler och blueprintens prompt.
...
## Steg 4 (rekommenderad drift): Batch blueprint + promptfiler

När preflight-filerna är metadata-patchade och SERP-berikade (`probes_completed >= 3`), generera blueprints i bulk:
...
Skriv artiklar per `job_NNN_prompt.md` och kör QA per artikel.

---

## Batch-mode

Det finns två giltiga batchprofiler:

1. **API-profil**: `pipe.run_batch_preflight(jobs)` + loop per jobb (kodexempel nedan)
2. **Script-profil (rekommenderad)**: `batch_preflight.py` → metadata/SERP-berikning → `batch_blueprint.py`
...
| Embedding-modell kan inte laddas (proxy/network/minne) | Systemet fallbackar till heuristisk semantic distance (0.5) och fortsätter körning |
...
*BACOWR v6.2 — RUNBOOK (uppdaterad 2026-02-19)*
