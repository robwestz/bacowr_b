...
input_fingerprint:
  ...
  included_files:
    ...
    - path: "qa-template.md"
      size: "~50 lines"
    - path: "text-output/runner.py"
      size: "~200 lines"
    - path: "text-output/qa_check.py"
      size: "~100 lines"
  excluded_patterns:
    - "node_modules/"
    - ".env*"
...
| serp_provider | serp_provider.py | External SERP API abstraction: Firecrawl and Ahrefs providers | Yes: `resolve_provider(name)` | Confirmed |
...
| runner | text-output/runner.py | Batch execution: job state machine, concurrency tracking, degraded mode detection | Yes: CLI entry, `parse_textjobs()` | Proposed |
| qa_check | text-output/qa_check.py | QA check implementation for 11-point article verification | Yes: QA runner | Proposed |
...
serp_provider.py (standalone, used by runner.py)
  |
  | resolve_provider() -> FirecrawlProvider | AhrefsProvider
  v
External SERP APIs (Firecrawl, Ahrefs)
```
