...
class TestSectionPlannerPlan:
    ...
    def test_robust_word_profile_and_anchor_center(self, planner, sample_thesis,
                                                   sample_topic, sample_bridge):
        sections = planner.plan(
            sample_thesis, sample_topic, [sample_bridge], "spela casino"
        )
        targets = [s.target_words for s in sections]
        assert targets == [130, 135, 135, 150, 145, 130]
        assert sum(targets) == 825

        words_before_anchor = sum(s.target_words for s in sections[:3])
        assert words_before_anchor == 400

    def test_two_trust_sections_are_before_anchor(self, planner, sample_thesis,
                                                  sample_topic, sample_bridge,
                                                  sample_bridge_secondary):
        sections = planner.plan(
            sample_thesis,
            sample_topic,
            [sample_bridge, sample_bridge_secondary],
        ...
        anchor_order = next(s.order for s in sections if s.contains_anchor)
        trust_orders = [s.order for s in sections if s.contains_bridge]

        assert trust_orders == [2, 3]
        assert all(order < anchor_order for order in trust_orders)
...
class TestConstraintEnforcerCheckBlueprint:
    ...
    def test_trust_after_anchor_fails(self, enforcer, complete_blueprint):
        complete_blueprint.sections[1].contains_bridge = None
        complete_blueprint.sections[4].contains_bridge = "bridge_001"
        results = enforcer.check_blueprint(complete_blueprint)
        trust_pos = next(r for r in results if r.name == "trust_before_anchor_section")
        assert trust_pos.passed is False
