...
try:
    ...
    print(
        ...
        f"({exc.__class__.__name__}). Semantic distance disabled."
    )
    ...
    def _get_model(self):
        ...
        if self._model is None and EMBEDDINGS_AVAILABLE:
            ...
            try:
                ...
                # Degrade gracefully when model download/load fails in locked-down environments.
                print(
                    "WARNING: embedding model load failed "
                    f"({exc.__class__.__name__}). Falling back to heuristic distance."
                )
                self._model = False
                return None
        return self._model
...
class PromptGenerator:
    ...
    Deprecated in v6.2: use engine blueprint prompts via `bp.to_agent_prompt()`.
    """

    def generate(self, preflight: Preflight) -> str:
        """Create a complete article generation prompt."""
        warnings.warn(
            "PromptGenerator is deprecated. Use blueprint.to_agent_prompt() instead. "
            "See FLOWMAP.md Phase 6.",
            DeprecationWarning,
            stacklevel=2,
        )
        job = preflight.job
...
class Pipeline:
    ...
    def save_prompt(self, preflight: Preflight):
        ...
        warnings.warn(
            "Pipeline.save_prompt() is deprecated. Use blueprint.to_agent_prompt() "
            "and save that output instead. See FLOWMAP.md Phase 6.",
            DeprecationWarning,
            stacklevel=2,
        )
